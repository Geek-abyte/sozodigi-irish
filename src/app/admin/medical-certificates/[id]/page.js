    ...
    <div className="w-full px-2 sm:px-4 md:px-6 py-10 print:p-0 print:bg-white">
      ...
      <div className="flex justify-center">
        <div className="bg-white w-full max-w-[794px] border shadow print:shadow-none print:border-none p-6 sm:p-10 text-gray-900 relative">
          {/* Watermark */}
          <img src="/images/logo/icon.png" className="absolute inset-0 w-full h-full opacity-5 pointer-events-none object-contain" alt="Watermark" />
          ...
          <div className="border-4 border-[#9bb5b4] p-4 sm:p-6 md:p-10 relative">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-gray-600">11 The Avenue Folkstown Park</p>
              <p className="text-sm text-gray-600 mb-2">Balbriggan Co Dublin, A92 XKR8</p>
              <p className="text-sm text-gray-600">Phone: 0419804419 | Fax: 0419803286</p>
            </div>
            ...
            <div className="mb-6 text-left">
              <p className="text-sm mb-1"><strong>Patient Name:</strong> {patientName}</p>
              <p className="text-sm mb-1"><strong>Address:</strong> {patientAddressLine}</p>
              <p className="text-sm"><strong>Date:</strong> {new Date(issueDate).toLocaleDateString("en-GB")}</p>
            </div>

            {/* Body */}
            <div className="leading-7 mb-8 text-sm sm:text-base text-justify">
              <p className="mb-4">
                This is to certify in my opinion that the above named patient is/was suffering from <strong>{diagnosis}</strong>
              </p>
              ...
              <div className="flex gap-8 mb-4">
                <p>From: <strong>{new Date(validFrom).toLocaleDateString("en-GB")}</strong></p>
                <p>To: <strong>{new Date(validTo).toLocaleDateString("en-GB")}</strong></p>
              </div>
            ...
            <div className="mt-8 border-t pt-6">
              <div className="flex justify-between items-end">
                <div className="flex-1">
                  <div className="mb-4">
                    <img
                      src={qrCodeUrl ? `${API_URL}${qrCodeUrl}` : "/images/qrcode.png"}
                      alt="QR Code - Doctor Profile"
                      className="w-20 h-20"
                    />
                    <p className="text-xs text-gray-500 mt-1">Scan to verify doctor</p>
                  </div>
                </div>

                <div className="flex-1 text-right">
                  <div className="mb-2">
                    <p className="text-sm mb-1">Signed: ___________________________</p>
                    <p className="font-semibold text-base">{doctorName}</p>
                    <p className="text-sm text-gray-600">MCN: {doctorRegistration}</p>
                  </div>
                </div>
              </div>
