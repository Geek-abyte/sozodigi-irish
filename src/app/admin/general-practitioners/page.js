...
export default function GeneralPractitionersPageContent() {
  ...
  const loadGeneralPractitioners = async () => {
    ...
    try {
      ...
      const gpsOnly = res.filter((doctor) =>
        (doctor.category || "").trim().toLowerCase() === "general practitioner"
      );
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:bg-gray-900 dark:text-gray-300 p-6">
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">View all general practitioners available in the system</p>
        </div>
        <button 
      ...
      {loading ? (
          ...
          {generalPractitioners.map((gp, index) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-[var(--color-primary-6)] text-sm font-medium mb-3">{gp.category || "General Practitioner"}</p>
        ...
        <div className="text-center py-12">
          <p className="text-gray-500 dark:text-gray-400">No general practitioners found.</p>
        </div>
      )}

      {/* GP Profile Modal */}
      {selectedGP && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={closeModal}>
          <motion.div 
            ...
            <div className="relative p-8">
              ...
              <div className="flex flex-col md:flex-row gap-8">
                <div className="md:w-1/3 flex flex-col items-center text-center">
                  <img
                    src={selectedGP.profileImage ? `${apiUrl}${selectedGP.profileImage}` : defaultUser.src}
                  ...
                  <p className="text-[var(--color-primary-6)] dark:text-[var(--color-primary-4)] font-medium mb-6">{selectedGP.category || "General Practitioner"}</p>
                  
                  <div className="w-full space-y-4 text-left">
                    {selectedGP.qualifications && (
                      <div>
                        <h3 className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Qualifications</h3>
                        <p className="text-gray-700 dark:text-gray-300">{selectedGP.qualifications}</p>
                      </div>
                    )}
                    
                    {selectedGP.yearsOfExperience && (
                      <div>
                        <h3 className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Experience</h3>
                        <p className="text-gray-700 dark:text-gray-300">{selectedGP.yearsOfExperience} years</p>
                      </div>
                    )}
                    
                    {selectedGP.location && (
                      <div>
                        <h3 className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Location</h3>
                        <p className="text-gray-700 dark:text-gray-300">{selectedGP.location}</p>
                      </div>
                    )}

                    <div>
                      <h3 className="text-sm uppercase tracking-wider text-gray-500 dark:text-gray-400 font-semibold mb-1">Status</h3>
                      <p className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                        selectedGP.approvalStatus === 'approved' || selectedGP.isApproved
                          ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'
                          : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200'
                      }`}>
                        {selectedGP.approvalStatus 
                          ? selectedGP.approvalStatus.charAt(0).toUpperCase() + selectedGP.approvalStatus.slice(1)
                          : (selectedGP.isApproved ? "Approved" : "Pending")
                        }
                      </p>
                    </div>
                  </div>
                ...
                <div className="md:w-2/3 dark:text-gray-100">
                  <div className="mb-6">
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">About Dr. {selectedGP.firstName} {selectedGP.lastName}</h3>
                  ...
                  {selectedGP.services && selectedGP.services.length > 0 && (
                    <div className="mb-6">
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">Services Offered</h3>
                  ...
                  <div className="mb-6">
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">Specialization</h3>
                    <p className="text-gray-600 dark:text-gray-300">
                      {selectedGP.category || "General Practitioner"} - Providing comprehensive primary healthcare services including routine check-ups, diagnosis, treatment, and preventive care.
                    </p>
                  </div>

                  {selectedGP.specialty && (
                    <div className="mb-6">
                      <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-3">Specialty</h3>
                      <p className="text-gray-600 dark:text-gray-300">{selectedGP.specialty}</p>
                    </div>
                  )}
                </div>
              </div>
