  ...
  const fetchAvailableSlots = async () => {
    ...
    try {
      ...
      console.log("Fetching slots for:", {
        selectedDayName,
        selectedDateString,
        specialistsCount: specialistsByCategory.length,
      });
      ...
      for (const specialist of specialistsByCategory) {
        ...
        console.log(
          `Specialist ${specialist.firstName} ${specialist.lastName} - Total slots:`,
          slots.length,
        );
        ...
        const filtered = slots.filter((slot) => {
          ...
          const expectedCategory =
            !showSpecialistCategories && !selectedCategory ? "cert" : "general";
          if (slot.category !== expectedCategory) {
            console.log(
              `Slot category mismatch: expected ${expectedCategory}, got ${slot.category}`,
              slot,
            );
            return false;
          }
          ...
          if (slot.type === "recurring") {
            ...
            if (!dayMatch) {
              console.log(
                `Recurring slot day mismatch: slot.dayOfWeek="${slot.dayOfWeek}", selectedDayName="${selectedDayName}"`,
                slot,
              );
            }
            return dayMatch;
            ...
            if (!dateMatch) {
              console.log(
                `One-time slot date mismatch: slotDate="${slotDateStr}", selectedDate="${selectedDateString}"`,
                slot,
              );
            }
            return dateMatch;
        ...
        console.log(
          `Specialist ${specialist.firstName} - Filtered slots:`,
          filtered.length,
        );
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-0">
      <h2 className="text-2xl font-bold mb-2">
        Book {selectedCategory} Consultation Appointment
      </h2>
      <BookingInstructions
      ...
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 [${showSpecialistCategories ? "lg:grid-cols-3" : ""}] gap-3`}
      >
        <div>
                ...
                {availableSlots.map((slot) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-600">
                      Consultant: {slot.consultant.firstName}{" "}
