  ...
  return (
    <Dialog title="Lab Referrals" onClose={() => setShowReferrals(false)}>
          ...
          {labReferrals.map((ref, index) => (
            ...
            >
              <div className="text-sm">
                <p>
                  <strong>{ref.testName}</strong> ({ref.status})
                </p>
                {ref.labName && (
                  <p className="text-gray-600">Lab: {ref.labName}</p>
                )}
                {ref.note && (
                  <p className="text-gray-500 text-xs">{ref.note}</p>
                )}
              </div>
              <button
      ...
      <input
        ...
        onChange={(e) =>
          setNewReferral({ ...newReferral, testName: e.target.value })
        }
      />
      <input
        className="w-full mb-2 p-2 border rounded"
        ...
        onChange={(e) =>
          setNewReferral({ ...newReferral, labName: e.target.value })
        }
      />
      <input
        className="w-full mb-2 p-2 border rounded"
        ...
        onChange={(e) =>
          setNewReferral({ ...newReferral, note: e.target.value })
        }
      />
      <select
        className="w-full mb-2 p-2 border rounded"
