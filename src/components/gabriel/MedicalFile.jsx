...
import {
  fetchMedicalFile,
  updateMedicalFile,
} from "../states/medicalFileSlice";
import { FaChevronDown, FaChevronUp } from "react-icons/fa";

const MedicalFile = ({ patientId }) => {
  const dispatch = useDispatch();
  const { medicalFile, loading, error } = useSelector(
    (state) => state.medicalFile || {},
  );
  ...
  const handleSubmitNote = () => {
    if (newNote.trim()) {
      dispatch(updateMedicalFile({ patientId, content: newNote })).then(
        (action) => {
  ...
  const handleSubmitPrescription = () => {
    if (
      ...
      dispatch(
        updateMedicalFile({ patientId, prescription: newPrescription }),
      ).then((action) => {
        if (updateMedicalFile.fulfilled.match(action)) {
          dispatch(fetchMedicalFile(patientId));
        }
        ...
        <div className="p-4">
          ...
          {activeTab === "info" && medicalFile?.patientInfo && (
            <div className="space-y-2">
              <InfoItem
                label="Name"
                value={`${medicalFile.patientInfo.firstName} ${medicalFile.patientInfo.lastName}`}
              />
              <InfoItem
                label="Date of Birth"
                value={
                  medicalFile.patientInfo.dateOfBirth
                    ? new Date(
                        medicalFile.patientInfo.dateOfBirth,
                      ).toLocaleDateString()
                    : "N/A"
              ...
              <InfoItem
                label="Gender"
                value={medicalFile.patientInfo.gender || "N/A"}
              />
              <InfoItem
                label="Phone"
                value={medicalFile.patientInfo.phone || "N/A"}
              />
              <InfoItem
                label="Email"
                value={medicalFile.patientInfo.email || "N/A"}
              />
              <InfoItem
                label="Allergies"
                value={medicalFile.patientInfo.allergies || "None"}
              />
              <InfoItem
                label="Medications"
                value={medicalFile.patientInfo.medications || "None"}
              />
              <InfoItem
                label="Conditions"
                value={medicalFile.patientInfo.conditions || "None"}
              />
            </div>
          )}
                ...
                {medicalFile?.notes?.map((note, index) => (
                  <div key={index} className="mb-2 p-2 bg-gray-700 rounded">
                    <p className="text-sm text-gray-300">
                      ...
                      {note.specialistId.firstName} {note.specialistId.lastName}{" "}
                      ({note.specialistId.specialistCategory})
                    </p>
                ...
                {medicalFile?.prescriptions?.map((prescription, index) => (
                  <div key={index} className="mb-2 p-2 bg-gray-700 rounded">
                    <p className="text-sm text-gray-300">
                      ...
                      {prescription.specialistId.firstName}{" "}
                      {prescription.specialistId.lastName} (
                      {prescription.specialistId.specialistCategory})
                    </p>
