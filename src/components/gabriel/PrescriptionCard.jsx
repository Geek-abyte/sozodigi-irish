...
const PrescriptionCard = ({ prescription, className, onClick }) => {
  ...
  const doctorName = specialistId
    ? `${specialistId.firstName} ${specialistId.lastName}`
    : "Unknown Doctor";
  const doctorImage = specialistId?.profileImage || defaultUser;

  return (
    <div
      className={`bg-white rounded-lg shadow-md p-4 cursor-pointer ${className}`}
      onClick={onClick}
    >
      <div className="flex items-center mb-4">
        ...
        <div>
          ...
          <div className="text-gray-500">
            {specialistId?.specialistCategory || "Doctor"}
          </div>
        </div>
      </div>
